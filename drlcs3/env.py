...
class ReservoirEnv(gym.Env):
    ...
    # Assign the initial well_observations to the first row
        ...
        obs = {
            "res_state": three_d_image,  # Replace with actual data
            "well_observations": well_observations_selected  # Replace with actual data
        }
    ...
    def apply_mask(self, res_state, ratio):
        """
        Gradually mask a portion of non-zero elements in res_state based on the ratio.

        Parameters:
        - res_state (np.ndarray): The reservoir state to be masked.
        - ratio (float): The proportion of non-zero elements to mask (0.0 to 1.0).

        Returns:
        - np.ndarray: The masked reservoir state.
        """
        if ratio <= 0.0:
            return res_state  # No masking
        elif ratio >= 1.0:
            return np.zeros_like(res_state)  # Full masking

        # Identify non-zero elements
        non_zero_indices = np.nonzero(res_state)
        total_non_zero = len(non_zero_indices[0])

        if total_non_zero == 0:
            return res_state  # Nothing to mask
        ...
        mask_indices = tuple(
            idx[np.random.choice(total_non_zero, num_to_mask, replace=False)]
            for idx in non_zero_indices
        )

        # Create a copy to avoid modifying the original observation
        masked_res_state = res_state.copy()
        masked_res_state[mask_indices] = 0.0
