...
RAMDISK_BASE = "/dev/shm/drlcs2"  # Where we want our env's working directory
SOURCE_DIR = "/home/ubuntu/clrm/drlcs2"  # Where TRUE_DECK0R.DATA and TRUE_DECK0.DATA are located
...
class ReservoirEnv(gym.Env):
    def __init__(self,env_id):
        ...
        self.initial_observation = np.zeros((9, 30), dtype=np.float32)
        ...
        self.observation_space = spaces.Dict({
            "well_observations": spaces.Box(
                low=0, high=np.inf, shape=(9,30), dtype=np.float32
            ),
            "history": spaces.Box(
        ...
        if int(self.env_id)==76:  # Testing case, use TRUE_DECK
