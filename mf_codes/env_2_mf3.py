...
import json
import torch
RAMDISK_BASE = "/dev/shm/drlcs3"  # Where we want our env's working directory
SOURCE_DIR = "/home/ubuntu/clrm/drlcs3"  # Where TRUE_DECK0R.DATA and TRUE_DECK0.DATA are located


class ReservoirEnv(gym.Env):
    def __init__(self,env_id,masking_ratio):
        ...
        if int(self.env_id)==151:  # Testing case, use TRUE_DECK
    ...
    def get_observation(self, step):
        ...
        if self.masking_ratio.value > 0:
            obs['res_state'] = self.apply_mask(obs['res_state'], self.masking_ratio.value)
        obs['res_state'] = obs['res_state'].clone().detach().float()
        return obs

    def apply_mask(self, res_state, ratio):
        """
        Gradually mask a portion of non-zero elements in a PyTorch tensor `res_state`
        based on the ratio.

        Parameters:
        - res_state (torch.Tensor): The reservoir state to be masked.
        - ratio (float): The proportion of non-zero elements to mask (0.0 to 1.0).

        Returns:
        - torch.Tensor: The masked reservoir state.
        """
        if ratio <= 0.0:
            # No masking
            return res_state
            ...
            # Full masking
            return torch.zeros_like(res_state)

        # Identify non-zero elements. 'as_tuple=True' returns tuple of index tensors
        non_zero_indices = torch.nonzero(res_state, as_tuple=True)
        total_non_zero = non_zero_indices[0].size(0)
        ...
        num_to_mask = int(total_non_zero * ratio)

        # Randomly select which non-zero elements to mask
        perm = torch.randperm(total_non_zero, device=res_state.device)[:num_to_mask]

        # Clone so we don't modify the original
        masked_res_state = res_state.clone()

        # For an N-dimensional tensor, non_zero_indices will be a tuple of length N,
        # each containing the indices along one dimension. We just pick the subset
        # for the randomly chosen positions and index directly.
        coords_to_mask =[idx[perm] for idx in non_zero_indices]

        # Now index into the masked_res_state using all dimensions simultaneously
        masked_res_state[tuple(coords_to_mask)] = 0.0

        return masked_res_state
