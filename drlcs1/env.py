...
class ReservoirEnv(gym.Env):
    def __init__(self,env_id):
        ...
        self.action_low = np.array([5, 5, 5,10,10,10], dtype=np.float32)
        self.action_high = np.array([60, 60, 60,100,100,100], dtype=np.float32)
        ...
        source_file1 = 'TRUE_DECK0R.DATA'
        source_file2 = 'TRUE_DECK0.DATA'
    ...
    def compute_reward(self, obs,step):
        ...
        # Calculate NPV
        reward = (np.sum(co2_inflow_present_values) - np.sum(co2_outflow_present_values) - np.sum(brine_treatment_present_values))/1e07
