...
from env_2_mf1 import ReservoirEnv  # Ensure this module is correctly implemented
...
class NestedDataParallelNet(DataParallelNet):
    def forward(self, obs, *args, **kwargs):
        # Get the device of the current module (varies per replica)
        device = next(self.net.parameters()).device
        
        # Move obs tensors to the correct device for this replica
        obs = {
            k: (
                v.to(device) if isinstance(v, torch.Tensor)
                else torch.as_tensor(v, dtype=torch.float32).to(device)
            )
            for k, v in obs.items()
        }
        return self.net(obs=obs, *args, **kwargs)  # Pass obs positionally

class CustomDictDataParallel(nn.Module):
    def __init__(self, module: nn.Module):
        super().__init__()
        self.dp_module = nn.DataParallel(module) # Wrap Critic/Actor

    # This signature specifically matches the Critic call: critic(batch.obs, batch.act)
    # It assumes 'obs' is the first positional arg, 'action' is the second.
    def forward(self, obs: Dict[str, torch.Tensor], action: torch.Tensor, *args, **kwargs) -> Any:

        # No device handling here - assumes input tensors are on cuda:0.
        # nn.DataParallel handles scattering to replicas.

        # Unpack obs and pass its components PLUS action as keyword arguments.
        # nn.DataParallel will scatter all tensor values passed via kwargs.
        return self.dp_module(
            res_state=obs["res_state"],
            well_observations=obs["well_observations"],
            action=action, # Pass action explicitly so it gets scattered
             # We ignore *args and **kwargs here assuming critic call is simple.
             # If Actor also needs wrapping, a more complex forward is needed.
        )
...
buffer = VectorReplayBuffer(200000, len(train_envs))
