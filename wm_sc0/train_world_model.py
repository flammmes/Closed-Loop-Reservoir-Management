...
def main(args):
    ...
    if args.model_type == "geometric_gru":
        # load student decoder (trained on post-ft latents)
        decoder = Transposed3DCNN(latent_dim=latent_dim, out_channels=2).to(device)
        decoder.load_state_dict(torch.load(args.decoder_path, map_location=device))
        decoder.eval()
        if args.data_type == "student_pre_ft":
            # load pre→post mapping
            ...
            geom_decoder = PreToPostDecoder(mapping, decoder).to(device)
        else:
        ...
        for batch in pbar:
            ...
            if args.model_type in ["gru", "geometric_gru", "rssm"]:
                ...
                if args.model_type == "geometric_gru":
                    ...
                    # Flatten sequences into per-step transitions, as geometric loss is one-step
                    B, T, D = b_z_seq.shape
                    z_flat  = b_z_seq.reshape(B * T, D)
                    a_flat  = b_a_seq.reshape(B * T, -1)
                    r_flat  = b_r_seq.reshape(B * T, -1)
                    zn_flat = b_zn_seq.reshape(B * T, D)

                    h0 = torch.zeros(z_flat.size(0), gru_hidden_dim, device=device)
                    loss = ensemble.calculate_geometric_loss(
                        zn_flat, z_flat, a_flat, r_flat, h0, geom_decoder
                    )
...
if __name__ == "__main__":
    ...
    # data
    p.add_argument("--data_type", type=str, default="student_pre_ft",
    ...
    p.add_argument("--num_ensemble", type=int, default=10)
