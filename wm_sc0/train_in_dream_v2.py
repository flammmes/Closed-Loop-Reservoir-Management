...
from env_3_mb0 import ReservoirEnv
...
# --------------------------------------------------------------------------------------
# Schedule + fear (time-aware; vectorized; no Python `if` on tensors)
# --------------------------------------------------------------------------------------
def _apply_schedule_and_fear(raw_model: torch.Tensor, t_abs: torch.Tensor) -> torch.Tensor:
    ...
    # # Early-steps floor (fixed: tensor-safe)
    # early = (t <= 13)
...
# --------------------------------------------------------------------------------------
# Imagination (Dreamer-lite; terminal-aware using t0/max_steps)
# --------------------------------------------------------------------------------------
...
def imagine_ahead(
    ...
    # Per-ensemble hidden state (only if recurrent)
    ...
    for t in range(1, H + 1):
        ...
        # If no disagreement and you want max speed, you *could* keep old path.
        # This version always computes all models (simple + correct).
        ...
        for e, m in enumerate(models):
            ...
            R_list.append(re1.view(B, -1))  # [B,1]
        ...
        # Main rollout from chosen member

        z_next = Z.mean(dim=0)      # [B,D]
        raw_model = R.mean(dim=0)   # [B]

        # Disagreement (now consistent for recurrent WMs)
...
def dreamer_update(
    ...
    # Freeze WM params (keep graph wrt inputs)
...
def main(args):
    ...
    B = max(1, int(args.num_train_envs))          # imagination batch size
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 3) Dreamer-style imagination updates (fresh trajectories each update)
