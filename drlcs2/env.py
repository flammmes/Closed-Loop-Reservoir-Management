...
class ReservoirEnv(gym.Env):
    def __init__(self,env_id):
        ...
        self.action_low = np.array([1, 1, 1, 100], dtype=np.float32)
        self.action_high = np.array([60, 60, 60, 190], dtype=np.float32)

        # Define normalized action space between 0 and 1
        self.action_space = spaces.Box(low=0.0, high=1.0, shape=(4,), dtype=np.float32)
                               
        self.initial_observation = np.zeros((9, 15), dtype=np.float32)
        ...
        for _ in range(self.history_length):
            self.history.append(np.zeros_like(self.initial_observation.copy(), dtype=np.float32))


        # Define observation space
        self.observation_space = spaces.Dict({
            "well_observations": spaces.Box(
                low=0, high=np.inf, shape=(9, 15), dtype=np.float32
            ),
            "history": spaces.Box(
                low=-np.inf, high=np.inf, shape=(self.history_length, 9,15), dtype=np.float32
            ),
        })
    ...
    def reset(self):
        ...
        self.history.clear()
        for _ in range(self.history_length):
            self.history.append(np.zeros(self.initial_observation.copy(), dtype=np.float32))
    ...
    def step(self, action):
        ...
        try:
            ...
            self.raw_reward_log.append({"environment":self.env_id,"episode": self.episode_counter, "step": self.steps, "raw_reward": reward,"pen_reward":penalized_reward})
            done = self.steps >= self.max_steps
        except Exception as e:  # Catch any exceptions that might occur during the simulation
            print(f"Error during simulation: {e}")
            obs = {
                'well_observations': self.observation_space['well_observations'].sample(),
    ...
    def compute_reward(self, obs,step):
        ...
        # Calculate NPV
        reward = (np.sum(co2_inflow_present_values) - np.sum(co2_outflow_present_values) - np.sum(brine_treatment_present_values))/1e08
...
if __name__ == "__main__":
    ...
    for _ in range(5):  # Run for a few steps
        ...
        print(obs["well_observations"].shape, obs["history"].shape)
        print("Reward:", reward)
        print("Done:", done)
        if done:
            break
